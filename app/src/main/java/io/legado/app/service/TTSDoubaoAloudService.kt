...
class TTSDoubaoAloudService : BaseReadAloudService(), Player.Listener {
    ...
    private val tag = "TTSDoubaoAloudService"
    ...
    private val doubaoSpeakFetch = DoubaoSpeakFetch()
    ...
    private suspend fun getSpeakStream(speakText: String, fileName: String): String {
        ...
        try {
            return withContext(Dispatchers.IO) {
                val inputStream = doubaoSpeakFetch.synthesizeText(ctx, speakText, speechRate)
            ...
            Log.i(tag, "doubaoSpeakFetch失败: $e")
            cacheAudio(fileName, silentBytes)
        }
        return "fail"
    }
    ...
    override fun rewind15() {
        val target = (exoPlayer.currentPosition - 15000).coerceAtLeast(0L)
        if (target == 0L && exoPlayer.currentMediaItemIndex > 0) {
            exoPlayer.seekToDefaultPosition(exoPlayer.currentMediaItemIndex - 1)
            exoPlayer.seekTo(exoPlayer.duration - 15000)
        } else {
    ...
    override fun fastForward15() {
        val target = exoPlayer.currentPosition + 15000
        if (target > exoPlayer.duration && exoPlayer.hasNextMediaItem()) {
    ...
    override fun aloudServicePendingIntent(actionStr: String): PendingIntent? {
        return servicePendingIntent<TTSDoubaoAloudService>(actionStr)
    }
...
@UnstableApi
class DoubaoByteArrayDataSource(private val mediaMap: HashMap<String, ByteArray>) : DataSource {
...
@UnstableApi
class DoubaoByteArrayDataSourceFactory(private val mediaMap: HashMap<String, ByteArray>) :
    DataSource.Factory {
    override fun createDataSource(): DataSource {
        return DoubaoByteArrayDataSource(mediaMap)
    }
}
