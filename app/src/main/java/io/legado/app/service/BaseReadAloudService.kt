...
abstract class BaseReadAloudService : BaseService(),
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ...
            IntentAction.rewind15 -> rewind15()
            IntentAction.fastForward15 -> fastForward15()
    ...
    /**
     * 后退 15 秒，默认按段落回退，子类可覆盖为精确 seek
     */
    open fun rewind15() {
        repeat(3) { if (nowSpeak > 0) prevP() }
    }

    /**
     * 前进 15 秒，默认按段落前进，子类可覆盖为精确 seek
     */
    open fun fastForward15() {
        repeat(3) { if (nowSpeak < contentList.size - 1) nextP() }
    }
