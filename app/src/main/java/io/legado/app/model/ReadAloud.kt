...
object ReadAloud {
    ...
    private fun getReadAloudClass(): Class<*> {
        ...
        if (ttsEngine.contains("doubaoinner")) {
    ...
    fun rewind15(context: Context) {
        if (BaseReadAloudService.isRun) {
            val intent = Intent(context, aloudClass)
            intent.action = IntentAction.rewind15
            context.startForegroundServiceCompat(intent)
        }
    }

    fun fastForward15(context: Context) {
        if (BaseReadAloudService.isRun) {
            val intent = Intent(context, aloudClass)
            intent.action = IntentAction.fastForward15
            context.startForegroundServiceCompat(intent)
        }
    }
